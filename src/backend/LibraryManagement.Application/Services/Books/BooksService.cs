...
public class BooksService(ILibraryDbContext context) : IBooksService
{
    ...
    public async Task<ResultDto> Update(Guid bookId, UpdateBookDto dto)
    {
        var isValid = await new UpdateBookDtoValidator().ValidateAsync(dto);

        if (!isValid.IsValid)
            return ResultDto.Error(isValid.Errors.First().ErrorMessage);
    ...
    public ResultDto<IEnumerable<Genre>> ListGenres()
    {
        var genres = Enum.GetValues<Genre>();

        return new ResultDto<IEnumerable<Genre>>(genres);
    }
}